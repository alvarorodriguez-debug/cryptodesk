...
// ── Number formatting: thousands=comma, decimal=period ──────────────────────
function fmt(n, dec=2) {
  if (n==null||isNaN(+n)) return '0.' + '0'.repeat(dec)
  return Number(n).toLocaleString('en-US',{minimumFractionDigits:dec,maximumFractionDigits:dec})
}

// ── P&L: real gain/loss = exit value (USD) - entry value (USD) ───────────────
// Stored on operation at creation time for display without rate dependency
function computePnL(entMonto, salMonto, entMoneda, salMoneda) {
  // If same currency: trivial
  if (entMoneda && salMoneda && entMoneda.toUpperCase()===salMoneda.toUpperCase()) {
    return { monto: (parseFloat(salMonto)||0) - (parseFloat(entMonto)||0), moneda: entMoneda }
  }
  // Different currencies: return raw diff in entry moneda (operator's perspective)
  // The real USD P&L will be computed via live rates on dashboard
  return { monto: null, entMonto: parseFloat(entMonto)||0, salMonto: parseFloat(salMonto)||0, entMoneda, salMoneda }
}
...
const S = {
  ...
  calcTotal:{display:'flex',justifyContent:'space-between',padding:'8px 0 4px',fontSize:'14px',fontWeight:'600',borderTop:'0.5px solid var(--border2)',marginTop:'6px'},
  ...
  searchBox:{position:'relative',marginBottom:'8px'},
  searchInput:{width:'100%',padding:'8px 10px 8px 32px',borderRadius:'var(--radius)',border:'0.5px solid var(--border2)',background:'var(--bg3)',color:'var(--text)',fontSize:'13px'},
}

// ── Shared components (outside App = stable refs) ────────────────────────────
...
function InfoBox({label,value,style}){return <div style={{...S.infoBox,...style}}><div style={{fontSize:'11px',color:'var(--text2)',marginBottom:'3px'}}>{label}</div><div style={{fontSize:'14px',fontWeight:'500'}}>{value||'—'}</div></div>}
function CopyBtn({text}){
  const [copied,setCopied]=useState(false)
  function copy(){navigator.clipboard.writeText(text).catch(()=>{const t=document.createElement('textarea');t.value=text;document.body.appendChild(t);t.select();document.execCommand('copy');document.body.removeChild(t)});setCopied(true);setTimeout(()=>setCopied(false),2000)}
...
function ComboSelect({label,value,options,onChange,onAddCustom,placeholder}){
  ...
  function handleSelect(e){if(e.target.value==='__custom__'){setShowCustom(true);setCustomVal('');onChange('')}else{setShowCustom(false);onChange(e.target.value)}}
  function confirm(){const v=customVal.trim();if(!v)return;onAddCustom(v);onChange(v);setShowCustom(false)}
  return(<div><label style={S.lbl}>{label}</label><select value={showCustom?'__custom__':(value||'')} onChange={handleSelect}><option value="" disabled>Seleccionar...</option>{options.map(o=><option key={o} value={o}>{o}</option>)}<option value="__custom__">+ Agregar nuevo...</option></select>{showCustom&&<div style={{display:'flex',gap:'6px',marginTop:'6px'}}><input autoFocus value={customVal} onChange={e=>setCustomVal(e.target.value)} onKeyDown={e=>e.key==='Enter'&&confirm()} placeholder={placeholder||'Nuevo valor...'} style={{flex:1}}/><Btn sm variant="primary" onClick={confirm}>OK</Btn><Btn sm onClick={()=>{setShowCustom(false);onChange(options[0]||'')}}>✕</Btn></div>}</div>)
}

// Searchable account picker — handles 100+ accounts gracefully
function AccountPicker({label, cuentas, selected, color='green', onSelect}) {
  const [busq, setBusq] = useState('')
  const filtered = cuentas.filter(c =>
    c.nombre.toLowerCase().includes(busq.toLowerCase()) ||
    (c.moneda||'').toLowerCase().includes(busq.toLowerCase()) ||
    (c.tipo||'').toLowerCase().includes(busq.toLowerCase())
  )
  const selCuenta = cuentas.find(c=>c.id===selected)
  const borderColor = color==='green'?'var(--green)':color==='amber'?'var(--amber)':'var(--purple)'
  const bgColor = color==='green'?'var(--green-dim)':color==='amber'?'var(--amber-dim)':'var(--purple-dim)'
  return (
    <div style={{marginBottom:'14px'}}>
      <label style={{...S.lbl,marginBottom:'8px'}}>{label}</label>
      {selCuenta && (
        <div style={{padding:'8px 12px',borderRadius:'var(--radius)',border:`2px solid ${borderColor}`,background:bgColor,marginBottom:'8px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
          <div>
            <span style={{fontSize:'13px',fontWeight:'500'}}>{selCuenta.nombre}</span>
            <span style={{fontSize:'11px',color:'var(--text2)',marginLeft:'6px'}}>{selCuenta.tipo} · {selCuenta.moneda}</span>
            {selCuenta.saldo!=null&&<span style={{fontSize:'11px',color:'var(--text3)',marginLeft:'6px'}}>Saldo: {fmt(selCuenta.saldo,2)} {selCuenta.moneda}</span>}
          </div>
          <button onClick={()=>onSelect(null)} style={{background:'none',border:'none',cursor:'pointer',color:'var(--text3)',fontSize:'14px',padding:'2px 6px'}}>✕</button>
        </div>
      )}
      <div style={{position:'relative',marginBottom:'6px'}}>
        ...
        <input value={busq} onChange={e=>setBusq(e.target.value)} placeholder="Buscar cuenta..." style={{...S.searchInput}}/>
      </div>
      <div style={{maxHeight:'180px',overflowY:'auto',border:'0.5px solid var(--border)',borderRadius:'var(--radius)'}}>
        {filtered.length===0&&<div style={{padding:'12px',fontSize:'12px',color:'var(--text3)',textAlign:'center'}}>Sin resultados</div>}
        {filtered.map(c=>(
          <div key={c.id} onClick={()=>{onSelect(c.id);setBusq('')}}
            style={{padding:'8px 12px',cursor:'pointer',borderBottom:'0.5px solid var(--border)',background:selected===c.id?bgColor:'transparent',transition:'background .1s'}}
            onMouseEnter={e=>{if(selected!==c.id)e.currentTarget.style.background='var(--bg3)'}}
            ...
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              ...
              <div style={{textAlign:'right'}}>
                ...
                {selected===c.id&&<span style={{marginLeft:'6px',color:borderColor}}>✓</span>}
              </div>
            </div>
          </div>
        ))}
...
// ── Wizard steps at MODULE LEVEL — stable identity = no focus loss ────────────
...
// Auto-calc logic (all at module scope — no focus loss):
...
export default function App() {
  ...
  useEffect(()=>{fetchRates();const i=setInterval(fetchRates,5*60*1000);return()=>clearInterval(i)},[])

  async function fetchRates(){
    try{
      const r=await fetch('https://api.frankfurter.app/latest?from=USD&to=EUR,BRL,ARS,PYG,GBP,CLP,UYU')
      const j=await r.json()
      setRates({...j.rates,USD:1,USDT:1,USDC:1,DAI:1})
      setRatesTs(new Date().toLocaleTimeString('es-PY',{hour:'2-digit',minute:'2-digit'}))
    }catch(e){}
  }
  ...
  function toUSD(amount,currency){
    ...
    if(['USD','USDT','USDC','DAI'].includes(up)) return a
    const rate=rates[up]
    if(!rate) return null
    return a/rate
  }
  ...
  function confirmarPago(id){
    const op=operaciones.find(o=>o.id===id);if(!op)return
  ...
  function Dashboard(){
    ...
    let totalPnlUSD=0,volUSD=0,hasApprox=false
    concretadas.forEach(o=>{
      const p=pnlUSD(o)
      if(p===null)hasApprox=true;else totalPnlUSD+=p
      const v=toUSD(o.entrada.monto,o.entrada.moneda)
      if(v===null)hasApprox=true;else volUSD+=v
    })
    const pnlColor=totalPnlUSD>=0?'var(--green)':'var(--red)'
    return(
      <div>
        <div style={S.mGrid}>
          <div style={S.metric}>
            <div style={S.mLabel}>P&L Total (USD)</div>
            <div style={{...S.mVal,color:pnlColor}}>{hasApprox?'~':''}{totalPnlUSD>=0?'+':''}{fmt(totalPnlUSD,2)}</div>
            {ratesTs&&<div style={{fontSize:'10px',color:'var(--text3)',marginTop:'4px'}}>Tasas: {ratesTs}</div>}
          </div>
          <div style={S.metric}><div style={S.mLabel}>Volumen (USD)</div><div style={{...S.mVal,color:'var(--blue)'}}>{hasApprox?'~':''}{fmt(volUSD,2)}</div></div>
        ...
        {Object.keys(rates).length>0&&(
          <div style={S.card}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'10px'}}>
              <span style={{fontSize:'13px',fontWeight:'500'}}>Cotizaciones en vivo</span>
            ...
            <div style={{display:'flex',flexWrap:'wrap',gap:'8px'}}>
              {['EUR','BRL','ARS','PYG','GBP'].filter(k=>rates[k]).map(k=>(
                <div key={k} style={{background:'var(--bg3)',border:'0.5px solid var(--border)',borderRadius:'var(--radius)',padding:'8px 14px',minWidth:'110px'}}>
                  <div style={{fontSize:'10px',color:'var(--text3)',marginBottom:'2px'}}>1 USD =</div>
                  <div style={{fontSize:'14px',fontWeight:'500'}}>{fmt(rates[k],2)} {k}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={S.card}>
          ...
          {operaciones.length===0
            ...
            :[...operaciones].reverse().slice(0,6).map(o=>{
              const pnl=pnlUSD(o)
              const pnlRaw=o.pnlMonto??null
              const display=pnl!==null?{monto:pnl,label:'USD'}:{monto:pnlRaw,label:o.pnlMoneda||o.entrada.moneda}
              return(
                <div key={o.id} style={{...S.opRow,cursor:'pointer'}} onClick={()=>setModal({type:'verOp',id:o.id})} onMouseEnter={e=>e.currentTarget.style.background='var(--bg3)'} onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
                  ...
                  {display.monto!==null&&(
                    <div style={{textAlign:'right',marginRight:'8px',flexShrink:0}}>
                      <div style={{fontSize:'12px',fontWeight:'600',color:display.monto>=0?'var(--green)':'var(--red)'}}>{display.monto>=0?'+':''}{fmt(display.monto,2)} {display.label}</div>
                      <div style={{fontSize:'10px',color:'var(--text3)'}}>P&L</div>
                    </div>
                  )}
                  <Tag estado={o.estado}/>
                </div>
  ...
  function Operaciones(){
    ...
    return(
      <div>
        ...
        {filtered.length===0?<div style={S.empty}>Sin operaciones{filtroOps!=='todas'?' con estado '+filtroOps:''}</div>
          :[...filtered].reverse().map(o=>{
            ...
            return(
              <div key={o.id} style={{...S.opRow,cursor:'default'}} onMouseEnter={e=>e.currentTarget.style.background='var(--bg3)'} onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
                ...
                {display.monto!==null&&<div style={{textAlign:'right',marginRight:'8px',flexShrink:0}}><div style={{fontSize:'12px',fontWeight:'600',color:display.monto>=0?'var(--green)':'var(--red)'}}>{display.monto>=0?'+':''}{fmt(display.monto,2)} {display.label}</div></div>}
                <Tag estado={o.estado}/>
                <div style={{display:'flex',gap:'6px'}}>
  ...
  function VerOpModal({id}){
    const op=operaciones.find(o=>o.id===id);if(!op)return null
    ...
    const pnlUSDval=pnlUSD(op)
    const pnlRaw=op.pnlMonto??null
    const display=pnlUSDval!==null?{monto:pnlUSDval,label:'USD'}:{monto:pnlRaw,label:op.pnlMoneda||op.entrada.moneda}
    return(
      <div style={S.modalBg} onClick={()=>setModal(null)}>
        <div style={S.modalWide} onClick={e=>e.stopPropagation()}>
          ...
          {display.monto!==null&&(
            <div style={{background:display.monto>=0?'var(--green-dim)':'var(--red-dim)',border:`0.5px solid ${display.monto>=0?'var(--green-border)':'rgba(255,90,90,0.3)'}`,borderRadius:'var(--radius)',padding:'10px 14px',marginBottom:'14px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              <span style={{fontSize:'12px',color:display.monto>=0?'var(--green)':'var(--red)',fontWeight:'500'}}>Ganancia / Pérdida</span>
              <span style={{fontSize:'20px',fontWeight:'700',color:display.monto>=0?'var(--green)':'var(--red)'}}>{display.monto>=0?'+':''}{fmt(display.monto,2)} {display.label}</span>
            </div>
          )}
